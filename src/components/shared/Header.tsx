      ...
      <div className="flex gap-4">
        {TOP_BAR.map((item, index) => {
          if(item.subMenu && item.subMenu.length > 0){
            return <HeaderSubmenu subMenu={item.subMenu}/>
          }
          if(item?.icon){
            return <div className={`flex items-center ${item?.spacing ? "ml-8" : "" }`} onClick={() =>handleGoLink(item.link)}>
              <Button variant="outline" size="icon">{item.icon}</Button>
              </div>
          }
          return (
